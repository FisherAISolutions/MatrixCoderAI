...
// Placeholder session ID â€” only used when no auth session is available.
...
export default function ChatWorkspacePage() {
  ...
  // Lightweight activity feedback (e.g. "Building repo contextâ€¦",
  // "Searching embeddingsâ€¦", "Streaming responseâ€¦"). Owned here; passed
  // down to ChatPanel â†’ AgentStatusBar for display. Cleared on finish/error.
  ...
  useEffect(() => {
    ...
    const handoff = readMatrixProjectOpenHandoff(window.sessionStorage);
    if (!handoff) return;

    const snapshot = loadMatrixProjectWorkspaceSnapshot(window.localStorage);
    if (!snapshot) {
      toast.error('Project snapshot was not available.');
      return;
    }

    const context = loadMatrixProjectWorkspaceContext(window.localStorage);
    const restoredMessages = snapshot.chatMessages.map((message) => ({
      ...message,
      isStreaming: false,
    }));

    projectRestoreAppliedRef.current = true;
    setFileTree(snapshot.files);
    setMessages(restoredMessages);
    setMemoryStage(getMemoryStageRecommendation(restoredMessages.length));
    setSessionTokens(
      restoredMessages.reduce(
        (total, message) => total + (message.tokenCount ?? 0),
        0
      )
    );

    const activePath = snapshot.workspaceState?.activeFilePath;
    const restoredActiveFile = activePath
      ? findFileByPath(snapshot.files, activePath)
      : null;
    setActiveFile(restoredActiveFile);
    if (activePath && effectiveSessionId) {
      setStoredActiveFilePath(effectiveSessionId, activePath);
    ...
    toast.success(
      handoff.message ||
        `${context.currentProjectName ?? snapshot.name} loaded into Workspace.`
    );
  }, [effectiveSessionId]);
  ...
  useEffect(() => {
    const loadData = async () => {
      if (projectRestoreAppliedRef.current) {
        projectRestoreAppliedRef.current = false;
        return;
      }
      ...
      try {
        ...
        try {
          ...
          // Milestone B â€” restore active file from localStorage.
  ...
  // Milestone B â€” persist active file path per session, so a refresh
  ...
  // resets activeFile â†’ null. Because React batches both updates, this
  // effect would otherwise run once with (NEW sessionId, null activeFile)
  // and *clobber* whatever stored path the new session had â€” breaking the
  ...
  useEffect(() => {
    ...
    if (activeFileSessionRef.current !== effectiveSessionId) {
      // Session just changed â€” skip this run; loadData will restore the
  ...
  // ------------------------------------------------------------------
  // Message helpers â€” split into THREE distinct operations to prevent the
  // duplicate-assistant-message bug:
  //
  //   1. addMessage              â†’ UI append + DB persist (user msgs,
  //                                orchestrator routing msgs, etc.)
  //   2. appendMessageToUI       â†’ UI append ONLY  (streaming placeholder â€”
  //                                we don't want to save an empty content
  //                                row to the DB and then later save the
  //                                final content as a *different* row)
  //   3. persistAssistantMessage â†’ DB persist ONLY (called after streaming
  //                                completes â€” the message already lives in
  ...
  //
  // PATCH PIPELINE NOTE â€” this is the SINGLE point where AI-applied
  ...
  //                  context-build pass â€” so EVERY patch must update it,
  //                  not just the currently-open file)
  //   2. activeFile (used by both <FileViewer/> instances â€” the one in
    ...
    saveFile(
      ...
      .then((result) => {
        if (
          ...
          toast.error(`Could not save ${file.name} â€” DB unavailable`);
        }
      })
      .catch((err) => {
  ...
  const addFile = useCallback((file: FileNode) => {
    ...
    if (collided) {
      console.warn('[addFile] path collision â€” refusing to duplicate', file.path);
      toast.error(`A file already exists at ${file.path}`);
      return;
    }

    // Save to Supabase â€” hardening pass #7: visible diagnostics on failure.
    ...
    saveFile(effectiveSessionId, file.path, file.name, file.content || '', file.language).then(result => {
      if (result) {
        ...
        // Phase 2B â€” embed in background after successful save
        if (file.content) {
  ...
  const createNewFile = useCallback((name: string) => {
    ...
    // never get to the `+ button â†’ addFile â†’ toast error` round-trip.
  ...
  /**
   * Milestone C â€” lightweight, same-folder rename.
   *
   ...
   *
   * Cross-folder moves are intentionally NOT supported here â€” the input
      ...
      renameFileInDb(file.id, newPath, newName, parentPath)
        .then((result) => {
          if (result) {
            ...
            console.warn('renameFile returned null â€” DB may be unavailable');
  ...
  // Parses zip â†’ creates a new session named after the project â†’ bulk-saves files â†’ switches session.
  const runZipImport = useCallback(async (zipFile: File) => {
    ...
    try {
      ...
      setImportSummary(summaryParts.join(' Â· '));
      setImportPhase('done');

      toast.success(`Imported "${projectTitle}" â€” ${saved} files`);
      ...
      // Phase 2B â€” background embeddings indexing (fire-and-forget).
      ...
      if (saved > 0) {
        ...
        indexProject(newSession.id, indexInputs)
          .then((r) => {
            if (r.disabled) {
              console.info('[indexer] pgvector unavailable â€” using heuristic-only context');
            } else {
  ...
  // Reuses the EXACT same downstream pipeline (bulkSaveFiles â†’ reload â†’
  // tree build â†’ background indexing) so behaviour and UI feedback are
  ...
  const runGithubImport = useCallback(async (repoUrl: string, ref?: string) => {
    ...
    try {
      ...
      // reuses these phase names; "reading" â†’ talking to GitHub,
      // "parsing" â†’ walking the tree and pre-screening blobs.
      setImportPhase('reading');
      await new Promise((r) => setTimeout(r, 50));
      setImportPhase('parsing');
...
/>
           ...
           *  0 intrinsic height, so its `absolute â€¦ h-[60%]` rendered to
      ...
      {/* Phase 3 â€” Terminal/Runtime Panel.
